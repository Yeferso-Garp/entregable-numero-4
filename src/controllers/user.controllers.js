...
await sendEmail({
    to: email,
    subject: 'Verificate email for users',
    html:`
...
const login = catchError(async(req, res) => {
    ...
    if (!user) return res.status(401).json({message: "Invalit credentials "});
    if (!user.isVerified) return res.status(401).json({message: "User not verified"});
    const isValit = await bcrypt.compare(password, user.password);
    if (!isValit) return res.status(401).json({message: "Invalit credentials"});
